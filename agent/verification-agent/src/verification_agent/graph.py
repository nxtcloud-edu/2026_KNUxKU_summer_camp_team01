...
def _build_findings(checks: VerificationChecks) -> VerificationFindings:
    ...
    for check_name, check in (
        ...
        items.extend(_standard_findings(check_name, check))
    ...
    for issue in checks.duration_realism.issues:
        level = "위험" if issue.severity == "fail" else "주의"
        items.append(
            VerificationFinding(
                level=level,
                ...
                message=issue.message,
                attention=(
                    f"체류시간을 {issue.suggested_min}~{issue.suggested_max}분으로 "
                    "조정할지 확인하세요."
                    if level == "주의"
                    else None
                ),
