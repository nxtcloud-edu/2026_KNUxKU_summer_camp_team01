...
async def health() -> dict[str, str]:
    return {"status": "ok", "service": "search", "provider": "request-controlled"}
...
async def search(request: Request) -> StreamingResponse:
    ...
    async def events() -> AsyncIterator[str]:
        ...
        try:
            ...
            response_mode = str(
                body.pop("response_mode", "handoff") or "handoff"
            ).strip()
            if response_mode not in {"handoff", "candidates"}:
                yield emitter.event(
                    "error",
                    code="invalid_input",
                    message="response_mode은 handoff 또는 candidates여야 해요.",
                    retryable=False,
                )
                return
            ...
            if response_mode == "candidates":
                ...
                yield emitter.event(
                    ...
                    summary="검색 후보를 사용자 선택 화면에 전달했어요.",
                )
                return
