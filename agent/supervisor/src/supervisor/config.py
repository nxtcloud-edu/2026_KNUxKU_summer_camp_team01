...
def _build() -> SupervisorConfig:
    ...
    search_provider_mode = _get("SEARCH_PROVIDER_MODE", "hybrid").casefold()
    if search_provider_mode not in {"mock", "hybrid"}:
        logger.warning(
            "SEARCH_PROVIDER_MODE=%r은 지원하지 않습니다. hybrid를 사용합니다.",
    ...
    return SupervisorConfig(
        ...
        hard_timeout_ms=_get_int("AGENT_HARD_TIMEOUT_MS", 180_000),
        ...
        search_timeout_ms=_get_int("SEARCH_AGENT_TIMEOUT_MS", 120_000),
...
def describe() -> list[str]:
    return [
        ...
        f"search provider: {CONFIG.search_provider_mode}",
