...
모든 에이전트의 오케스트레이션은 **LangGraph**로 구현한다.

| 항목 | 결정 |
|---|---|
| 오케스트레이션 프레임워크 | **LangGraph** |
| 적용 범위 | `cityInfo` · `flightSearch` · `staySearch` · `placeDiscovery` · `itineraryGenerate` · `itineraryVerify` 전체 |
| 그래프 구성 | 작업별로 상태(State)와 노드(Node)를 정의한다 |
| 구현 재량 | 노드 분리 방식·도구 구성·LLM 모델·프롬프트는 각 작업 담당자가 결정한다 |

LangGraph는 작업 흐름과 상태 전이를 관리한다. 날짜·시간·거리·가격 계산과 검증 규칙 판정처럼 정확성이 필요한 로직은 LangGraph 노드 안의 **결정론적 코드**로 구현하며, LLM에 맡기지 않는다.
...
| 완전히 자유 | 반드시 지킨다 |
|---|---|
| 언어·API 서버 프레임워크 (Python/TS/Go…) | **모든 에이전트의 오케스트레이션은 LangGraph 사용** |
| LLM 선택·모델·프롬프트 | 이벤트 스트림의 모양 ([`contract.md`](./contract.md)) |
| LangGraph 노드·상태·도구 구성 | 필수 필드와 형식 |
| 도구를 몇 개로 나눌지 | 종료 이벤트 보장 |
| 데이터 소스 | 한국어 카피 톤 · 목표 소요 시간 |
